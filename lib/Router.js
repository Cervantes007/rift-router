...
class Router {
  ...
  setActiveRoute(route, params = {}, search = {}) {
    var _a, _b;
    (_b = (_a = this.active) === null || _a === void 0 ? void 0 : _a.onLeave) === null || _b === void 0
      ? void 0
      : _b.call(_a, this);
    if (route.onEnter) {
  ...
  setRoutes(routes, components = [], parent = '', hooks = {}) {
    ...
    for (const route of routes) {
      ...
      if (children) {
        ...
        if (path === '*') {
          this.defaultRoute = Object.assign({ path: '*', components: components.concat(component) }, hook);
        } else {
          aux.push(
            Object.assign(
              { path: (parent + (path || '')).replace(/(\/)\1+/g, '/'), components: components.concat(component) },
              hook
            )
          );
        }
